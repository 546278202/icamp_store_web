...
export default {
  ...
  methods: {
    ...
    getList() {
      this.axios.get(this.Global.BASE_URL + `/category/${this.parentId}?page=${this.page}&size=10&type=2`).then(response => {
    ...
    handleProductCateDelete(index, row) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        this.axios.delete(this.Global.BASE_URL + `/category/${row.id}?type=2`).then(response => {
          if (response.data.status == 200) {
    ...
    handleShowStatusChange(e, index, row) {
      ...
      this.axios.patch(this.Global.BASE_URL + `/category/${row.id}?enable=${flag}&type=2`).then(response => {
        if (response.data.status == 200) {
          this.$message({ message: '恭喜你，这是一条成功消息', type: 'success' });
        }
      })
