<template> 
  <div class="detail-container" ref="productImage">
    <div>
      <el-steps :active="order.orderStatus" finish-status="success" align-center>
        <el-step title="提交订单" :description="formatTime(order.addDateTime)"></el-step>
    ...
    <el-card shadow="never" style="margin-top: 15px">
      <div class="operate-container">
        ...
        <!-- <div class="operate-button-container" v-show="order.orderStatus===5 && order.orderGoods[0].afterSales==null">
          ...
          <el-button size="mini" :order="order" @click="showExchangeApply">申请更换</el-button>
      ...
      <div class="table-layout">
        <el-row>
          <el-col :span="4" class="table-cell-title">营员</el-col>
          <el-col :span="4" class="table-cell-title">性别</el-col>
          <el-col :span="4" class="table-cell-title">身份号码</el-col>
          <el-col :span="4" class="table-cell-title">身高(cm)</el-col>
          <el-col :span="4" class="table-cell-title">体重(kg)</el-col>
          <el-col :span="4" class="table-cell-title">操作</el-col>
        </el-row>
        <el-row v-for="(item,index) in order.orderGoods" :key="index">
          <el-col :span="4" class="table-cell">{{item.camper.camperName}}</el-col>
          <el-col :span="4" class="table-cell">{{item.camper.sex | sexFilter}}</el-col>
          <el-col :span="4" class="table-cell">{{item.camper.identifications[0].idCode | formatNull}}</el-col>
          <el-col :span="4" class="table-cell">{{item.camper.weigth | formatNull}}</el-col>
          <el-col :span="4" class="table-cell">{{item.camper.weigth | formatNull}}</el-col>
          <el-col :span="4" class="table-cell"> <a class="color-blue" @click="$router.push({path:'/user/userDetail',query:{id:item.camper.userId}})">详情</a></el-col>
        </el-row>
      </div>
      <div style="margin-top: 20px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
      ...
      <el-table ref="orderItemTable" :data="order.orderGoods" style="width: 100%;margin-top: 20px" border>
        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <a class="color-blue">详情</a>
            <a class="color-blue" v-show="order.orderStatus===5 && order.orderGoods[0].afterSales==null" @click="showInitiateRefundDialog">退款</a>
            <a class="color-blue" v-show="order.orderStatus===5" @click="showExchangeApply(scope.$index, scope.row)">更换</a>
            <a class="color-blue" v-show="order.orderStatus===5 && order.orderGoods[0].afterSales==null" @click="showPostponeDialog">延期</a>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="修改费用信息" :visible.sync="moneyDialogVisible" width="40%">
      <div class="table-layout">
        ...
        <el-row>
          ...
          <el-col :span="6" class="table-cell">¥{{order.expressAmount}}</el-col>
          <el-col :span="6" class="table-cell">-￥{{order.couponDiscountAmount}}</el-col>
          <el-col :span="6" class="table-cell">-￥{{order.integralDiscountAmount}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="6" class="table-cell-title">活动优惠</el-col>
          <el-col :span="6" class="table-cell-title">折扣金额</el-col>
        ...
        <el-row>
          ...
          <el-col :span="6" class="table-cell">
            <el-input v-model.number="moneyInfo.amount" size="mini"><template slot="prepend">-￥</template></el-input>
          </el-col>
          <el-col :span="6" class="table-cell">
            <span class="color-danger">￥{{order.orderAmount}}</span>
          </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      moneyInfo: { orderId: null, amount: 0, type: null },
  ...
  methods: {
    ...
    showUpdateMoneyDialog() {
      ...
      this.moneyInfo.orderId = this.order.orderId;
      this.moneyInfo.amount = this.order.discountAmount;
    ...
    handleUpdateMoneyInfo() {
      this.$confirm('是否要修改费用信息?', '提示', {
        ...
        this.axios.put(this.Global.BASE_URL + `/order/adjustment`, this.moneyInfo).then(response => {
          console.log(response.data)
    ...
    showExchangeApply(index, item) {
      sessionStorage.setItem('orderExchange', JSON.stringify(item));
      console.log(JSON.parse(sessionStorage.getItem("orderExchange")))
      this.$router.push({ path: '/oms/orderExchange', query: { id: this.id } })
    },
